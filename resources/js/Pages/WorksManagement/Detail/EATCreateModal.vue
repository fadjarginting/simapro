...
<template>
    ...
    <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div class="bg-white rounded-xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col">
                        ...
                        <div class="flex items-center justify-between mb-8">
                            <div v-for="(step, index) in steps" :key="index" class="flex items-center">
                                <div class="flex items-center">
                                    <div :class="`w-10 h-10 rounded-full flex items-center justify-center font-bold text-white ${currentStep > index ? 'bg-green-500' :
                                ...
                                <div v-if="index < steps.length - 1" class="w-16 h-px bg-gray-300 mx-4"></div>
                            </div>
                        </div>
                        ...
                        <div class="bg-gray-50 rounded-lg p-6">
                            ...
                            <div v-show="currentStep === 0">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div>
                                        <label class="block text-sm font-medium mb-2 text-gray-700">Tanggal Mulai
                                            EAT</label>
                                        <input v-model="form.start_date" type="date"
                                            class="w-full bg-white border border-gray-300 rounded-lg px-3 py-2 text-gray-900 focus:outline-none focus:border-blue-500"
                                            required>
                                        <div v-if="form.errors.start_date" class="text-red-500 text-xs mt-1">{{
                                            form.errors.start_date }}</div>
                                    </div>
                                    <div>
                                        <label class="block text-sm font-medium mb-2 text-gray-700">Tanggal Selesai
                                            EAT</label>
                                    ...
                                    <div class="col-span-2 mt-2">
                                        <div class="text-sm text-gray-700 bg-gray-100 p-2 rounded-md">
                                            <span class="font-medium">Total Durasi EAT:</span>
                                            <span class="ml-1">{{ form.total_duration_days }} hari</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div v-show="currentStep === 1">
                                ...
                                <div class="space-y-4 max-h-96 overflow-y-auto">
                                    <div v-for="(activity, index) in form.activities" :key="index"
                                        class="bg-white rounded-lg p-4 border border-gray-200 relative">
                                        <button v-if="form.activities.length > 1" @click="removeActivity(index)"
                                            type="button"
                                            class="absolute top-4 right-4 text-red-500 hover:text-red-700">
                                            <i class="fas fa-trash text-sm"></i>
                                        </button>

                                        <h3 class="font-medium mb-4 text-gray-900">Aktivitas #{{ index + 1 }} (Urutan:
                                            {{ activity.sequence_order }})</h3>
                                        <div class="grid grid-cols-2 gap-4">
                                            <div>
                                                <label class="block text-sm font-medium mb-2 text-gray-700">Nama
                                                    Aktivitas</label>
                                                <input v-model="activity.activity_name" type="text"
                                                    class="w-full bg-white border border-gray-300 rounded-lg px-3 py-2 text-gray-900 focus:outline-none focus:border-blue-500"
                                                    placeholder="e.g., BOQ, Design, etc.">
                                                <div v-if="form.errors[`activities.${index}.activity_name`]"
                                                    class="text-red-500 text-xs mt-1">{{
                                                    form.errors[`activities.${index}.activity_name`] }}</div>
                                            </div>

                                            <div>
                                                <label
                                                    class="block text-sm font-medium mb-2 text-gray-700">Disiplin</label>
                                                <select v-model="activity.discipline_id"
                                                    class="w-full bg-white border border-gray-300 rounded-lg px-3 py-2 text-gray-900 focus:outline-none focus:border-blue-500">
                                                    <option value="">Pilih Disiplin</option>
                                                    <option v-for="discipline in disciplinesData" :key="discipline.id"
                                                        :value="discipline.id">
                                                        {{ discipline.name }}
                                                    </option>
                                                </select>
                                                <div v-if="form.errors[`activities.${index}.discipline_id`]"
                                                    class="text-red-500 text-xs mt-1">{{
                                                    form.errors[`activities.${index}.discipline_id`] }}</div>
                                            </div>

                                            <div class="col-span-2">
                                                <label
                                                    class="block text-sm font-medium mb-2 text-gray-700">Person-in-Charge
                                                    (PIC)</label>
                                                <div v-if="activity.discipline_id"
                                                    class="bg-gray-50 p-3 rounded-lg border border-gray-200 max-h-40 overflow-y-auto">
                                                    <div class="space-y-2">
                                                        <div v-for="user in getUsersForDiscipline(activity.discipline_id)"
                                                            :key="user.id" class="flex items-center space-x-2">
                                                            <input type="checkbox" :id="`pic-${index}-${user.id}`"
                                                                :value="user.id" v-model="activity.pic_ids"
                                                                class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 focus:ring-2">
                                                            <label :for="`pic-${index}-${user.id}`"
                                                                class="text-sm text-gray-700 cursor-pointer">
                                                                {{ user.name }}
                                                            </label>
                                                        </div>
                                                    </div>
                                                    <div v-if="getUsersForDiscipline(activity.discipline_id).length === 0"
                                                        class="text-sm text-gray-500 text-center py-2">
                                                        Tidak ada user tersedia untuk disiplin ini
                                                    </div>
                                                </div>
                                                <div v-if="activity.pic_ids.length > 0" class="mt-2">
                                                    <div class="text-sm text-gray-600 mb-1">PIC yang dipilih:</div>
                                                    <div class="flex flex-wrap gap-1">
                                                        <span v-for="picId in activity.pic_ids" :key="picId"
                                                            class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                                            {{ getUserName(picId) }}
                                                            <button
                                                                @click="activity.pic_ids = activity.pic_ids.filter(id => id !== picId)"
                                                ...
                                                <div v-if="!activity.discipline_id"
                                                    class="text-sm text-gray-500 mt-1 p-3 bg-gray-50 rounded-lg border border-gray-200">
                                                    Pilih disiplin terlebih dahulu untuk memilih PIC
                                                </div>
                                                <div v-if="form.errors[`activities.${index}.pic_ids`]"
                                                    class="text-red-500 text-xs mt-1">{{
                                                    form.errors[`activities.${index}.pic_ids`] }}</div>
                                            </div>
                                            <div>
                                                <label class="block text-sm font-medium mb-2 text-gray-700">Tanggal
                                                    Mulai
                                                    Aktivitas</label>
                                                <input v-model="activity.start_date" type="date"
                                                    class="w-full bg-white border border-gray-300 rounded-lg px-3 py-2 text-gray-900 focus:outline-none focus:border-blue-500">
                                                <div v-if="form.errors[`activities.${index}.start_date`]"
                                                    class="text-red-500 text-xs mt-1">{{
                                                    form.errors[`activities.${index}.start_date`] }}</div>
                                            </div>
                                            <div>
                                                <label class="block text-sm font-medium mb-2 text-gray-700">Tanggal
                                                    Selesai
                                                    Aktivitas</label>
                                                <input v-model="activity.end_date" type="date"
                                                    class="w-full bg-white border border-gray-300 rounded-lg px-3 py-2 text-gray-900 focus:outline-none focus:border-blue-500">
                                                <div v-if="form.errors[`activities.${index}.end_date`]"
                                                    class="text-red-500 text-xs mt-1">{{
                                                    form.errors[`activities.${index}.end_date`] }}</div>
                                            </div>
                                            <div class="col-span-2 mt-1">
                                                <div class="text-sm text-gray-700">
                                                    <span class="font-medium">Durasi aktivitas:</span>
                                                    <span class="ml-1">{{ activity.duration_days }} hari</span>
                                                </div>
                                            </div>
                                            <div class="md:col-span-2">
                                                <label class="block text-sm font-medium mb-2 text-gray-700">Deskripsi
                                                    Aktivitas</label>
                                                <textarea v-model="activity.activity_description" rows="3"
                                                    class="w-full bg-white border border-gray-300 rounded-lg px-3 py-2 text-gray-900 focus:outline-none focus:border-blue-500"
                                                    placeholder="Deskripsi singkat mengenai aktivitas"></textarea>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div v-show="currentStep === 2">
                                <h2 class="text-lg font-bold mb-4 text-gray-900">Langkah 3: Tentukan Approver per
                                    Disiplin
                                </h2>
                                <p class="text-gray-600 mb-6">
                                    Pilih lead atau manager yang bertanggung jawab untuk menyetujui pekerjaan dari
                                    setiap disiplin yang terlibat.
                                </p>
                                <div class="space-y-4">
                                    <div v-if="involvedDisciplines.length === 0"
                                        class="text-center text-gray-500 p-4 bg-gray-100 rounded-lg">
                                        Pilih disiplin pada aktivitas di langkah sebelumnya untuk menentukan approver.
                                    </div>
                                    <div v-for="discipline in involvedDisciplines" :key="discipline.id"
                                        class="bg-white rounded-lg p-4 border border-gray-200">
                                        <div class="flex flex-col md:flex-row justify-between md:items-center">
                                            <label class="block font-medium mb-2 md:mb-0 text-gray-700">
                                                Approver untuk Disiplin: <span class="font-bold">{{ discipline.name
                                                    }}</span>
                                            </label>
                                            <div class="w-full md:w-64">
                                                <select v-model="approvalSelections[discipline.id]"
                                                    class="w-full bg-white border border-gray-300 rounded-lg px-3 py-2 text-gray-900 focus:outline-none focus:border-blue-500">
                                                    <option :value="null">Pilih Lead/Manager</option>
                                                    <option v-for="lead in getLeadsForDiscipline(discipline.id)"
                                    ...
                                    <div v-if="form.errors.approvals" class="text-red-500 text-xs mt-2">{{
                                        form.errors.approvals }}</div>
                                </div>
                            </div>
                                ...
                                <div class="space-y-6">
                                    ...
                                    <div class="bg-white rounded-lg p-4 border border-gray-200">
                                        ...
                                        <div class="grid grid-cols-2 gap-4 text-sm">
                                            <div><span class="text-gray-600">Work ID:</span><span
                                                    class="ml-2 text-gray-900 font-semibold">{{ form.work_id }}</span>
                                            </div>
                                            <div><span class="text-gray-600">Total Durasi:</span><span
                                                    class="ml-2 text-gray-900 font-semibold">{{ form.total_duration_days
                                                    }}
                                                    hari</span></div>
                                            <div><span class="text-gray-600">Tanggal Mulai:</span><span
                                                    class="ml-2 text-gray-900 font-semibold">{{
                                                    formatDate(form.start_date) }}</span></div>
                                            <div><span class="text-gray-600">Tanggal Selesai:</span><span
                                                    class="ml-2 text-gray-900 font-semibold">{{
                                                    formatDate(form.end_date) }}</span></div>
                                        </div>
                                    </div>

                                    <!-- Ringkasan Aktivitas -->
                                    <div class="bg-white rounded-lg p-4 border border-gray-200">
                                        <h3 class="font-bold mb-4 text-gray-900">Ringkasan Aktivitas ({{
                                            form.activities.length }})</h3>
                                        <div class="space-y-3 max-h-48 overflow-y-auto">
                                            <div v-for="(activity, index) in form.activities" :key="index"
                                                class="bg-gray-50 rounded p-3 text-sm">
                                                <p class="font-semibold text-gray-900">{{ activity.sequence_order }}. {{
                                                    activity.activity_name }} ({{ activity.duration_days }} hari)</p>
                                                <p class="text-gray-600">Disiplin: <span class="font-medium">{{
                                                        getDisciplineName(activity.discipline_id) }}</span></p>
                                                <p class="text-gray-600">PIC: <span class="font-medium">{{
                                                        getPicNames(activity.pic_ids) }}</span></p>
                                                <p class="text-gray-600">{{ formatDate(activity.start_date) }} - {{
                                                    formatDate(activity.end_date) }}</p>
                                            </div>
                                        </div>
                                    </div>
            ...
            <!-- Modal Footer -->
            <div class="bg-gray-50 px-6 py-4 border-t border-gray-200 flex-none">
                <div class="flex justify-between">
                    ...
                    <div class="flex gap-2">
                        ...
                        <button v-if="currentStep === 3" @click="submitEAT(true)" type="button"
                            :disabled="form.processing"
                            class="px-6 py-2 bg-yellow-500 text-white rounded-lg hover:bg-yellow-600 transition disabled:opacity-50">
                            Simpan Draft
                        </button>
                        <button v-if="currentStep === 3" @click="submitEAT(false)" type="button"
                            :disabled="form.processing"
                            class="px-6 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition disabled:opacity-50">
                            {{ form.processing ? 'Mengirim...' : 'Kirim untuk Persetujuan' }}
                        </button>
                    </div>
                </div>
            </div>
