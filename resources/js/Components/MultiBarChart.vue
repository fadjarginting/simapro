...
const renderChart = () => {
  if (!chartContainer.value || !props.data || !props.data.categories || !props.data.series || props.data.series.length === 0) return;
  ...
  categoryGroups.selectAll('.bar-group')
    .data(category => {
      return series.map(s => {
